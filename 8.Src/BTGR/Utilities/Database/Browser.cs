...
namespace Utilities.Database
{
        ...
        public interface IBrowser
        {
            ...
            DataSet    GetSubObjectDataSet(TreeNode node);
            DataSet   Fields(string tablename);
            string[]   Tables{get;}
            ...
            /// </summary>
            IBrowser Clone (DbClientAsync newDbClient);
        }
        #endregion
        ...
        public class SqlBrowser : IBrowser
        {
            const int timeout = 5;
            //DbClient dbClient;
            DbClientAsync dbClient;
            public SqlBrowser (DbClientAsync dbClient)
            {
                this.dbClient = dbClient;
            }

            public DbClientAsync DbClient
            {
                get {return dbClient;}
            }
            ...
            public string[] Tables
            {
                get
                {
                    ...
                    DataSet ds = dbClient.Execute ("select type, ObjectProperty (id, N'IsMSShipped') shipped, object_name(id) object, user_name(uid) owner from sysobjects where type='U' order by object, owner", timeout);
                    string[] names=new string[ds.Tables[0].Rows.Count];
                    DataTable dt=ds.Tables[0];
                    for (int i=0;i<dt.Rows.Count;i++)
                    {
                        names[i]=Convert.ToString(dt.Rows[i][2]);
                    }
                    return names;
            ...
            public DataSet Fields(string tablename)
            {
                DataSet ds = dbClient.Execute ("select COLUMN_NAME name, DATA_TYPE type, CHARACTER_MAXIMUM_LENGTH clength, NUMERIC_PRECISION nprecision, NUMERIC_SCALE nscale, IS_NULLABLE nullable  from INFORMATION_SCHEMA.COLUMNS where TABLE_CATALOG = db_name() "
                    +  " and TABLE_NAME = '" + tablename + "' order by ORDINAL_POSITION", timeout);
                return ds;
            }
        ...
        public class OracleNode : TreeNode
        {
            internal string type = "";
        ...
        public class OracleBrowser : IBrowser
        {
            ...
            public string[] Tables
            {
                get
                {
                    ...
                    for (int i=0;i<dt.Rows.Count;i++)
                    {
                        names[i]=Convert.ToString(dt.Rows[i][0]);
                    }
                    return names;
            ...
            public DataSet  GetSubObjectDataSet(TreeNode  thenode)
            {
                if (thenode is OracleNode)
                {
				
				
                    OracleNode on = (OracleNode)thenode;

                    this.DbClient.Database=on.database;
                    if (on.type == "U" || on.type == "V")					// break down columns for user tables and views
                    {
