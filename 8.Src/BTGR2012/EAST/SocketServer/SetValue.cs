    ...
    public partial class SetValue : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            switch (tabControl1.SelectedTab.Text)
            {
                ...
                case "温度修正":
                    //2012
                    cmdindex = new byte[] { 32, 33, 34, 35,36 ,13};
                    ...
                    //{
                    //    MessageBox.Show("请选择某个温度修正再进行读取！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    //    return;
                    //}
                    ...
                    for (int j = 0; j < Tool.xd100x._XD100xBuffer.Length; j++)
                    {
                        if (id == Tool.xd100x._XD100xBuffer[j]._Info._id)
                        {
                            for (int k = 0; k < cmdindex.Length; k++)
                            {
                                ...
                                if (Tool.xd100x._XD100xBuffer[j]._Info._version == "xd100" && cmdindex[k] == 13)
                                {
                                    continue;
                                }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            switch (tabControl1.SelectedTab.Text)
            {
                ...
                case "温度修正":
                    //2012
                    cmdindex = new byte[] { 37, 38, 39, 40, 41,26};
                        ...
                        if (id == Tool.xd100x._XD100xBuffer[j]._Info._id)
                        {
                            ...
                            for (int k = 0; k < cmdindex.Length; k++)
                            {
                                ...
                                switch (cmdindex[k])
                                {
                                    ...
                                    case 26:
                                        if (Tool.xd100x._XD100xBuffer[i]._Info._version == "xd100n")
                                        {
                                            if ((DateTime.Now - Tool.xd100x._XD100xBuffer[j]._Command[13]._backdt).TotalMinutes > 60) continue;
                                        }
                                        break;
        ...
        private void reflashback()
        {
            for (int i = 0; i < Tool.xd100x._XD100xBuffer.Length; i++)
            {
                ...
                {
                    Tool.xd100x._XD100xBuffer[i]._Info._result = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " 读温度修正成功";
                ...
                if (Tool.xd100x._XD100xBuffer[i]._Command[13]._back == true)
                {
                    Tool.xd100x._XD100xBuffer[i]._Info._result = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " 读温度修正成功";
                    Tool.xd100x._XD100xBuffer[i]._Command[13]._back = false;
                    reflashflag = true;
                }
                ...
                {
                    Tool.xd100x._XD100xBuffer[i]._Info._result = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " 设置温度修正成功";
                    Tool.xd100x._XD100xBuffer[i]._Command[37]._back = false;
                    save_operate(Tool.xd100x._XD100xBuffer[i]._Info._id, "设置温度修正成功");
                    reflashflag = true;
                }
                ...
                if (Tool.xd100x._XD100xBuffer[i]._Command[26]._back == true)
                {
                    Tool.xd100x._XD100xBuffer[i]._Info._result = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " 设置温度修正成功";
                    Tool.xd100x._XD100xBuffer[i]._Command[26]._back = false;
                    save_operate(Tool.xd100x._XD100xBuffer[i]._Info._id, "设置温度修正成功");
                    reflashflag = true;
                }
        ...
        {
            string sql = "SELECT [DeviceID],[StationName] FROM [vDeviceGR] ";
            DataTable dt = Tool.DB.getDt(sql);
            comboBox1.DataSource = dt;
            comboBox1.DisplayMember = "StationName";
            comboBox1.ValueMember = "DeviceID";
            comboBox1.SelectedValue = SocketServer.Program.basedeviceid;
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            //修改基准站点
            xDoc.DocumentElement.ChildNodes[7].Attributes.GetNamedItem("value").Value = comboBox1.SelectedValue.ToString();
            SocketServer.Program.basedeviceid =Convert.ToInt32( comboBox1.SelectedValue.ToString());
