...
namespace Tool
{
    public class xd100x
    {
        ...
        //2012
        //循环命令生成
        public static void Polling_XD100xSetOutTemp()
        {
            Set _Set=new Set();
            _Set._outtemp._outtemp = SocketServer.Program.baseouttemp;
           // _Set._outtemp._outtemp = 9.1f;
            for (int i = 0; i < _XD100xBuffer.Length; i++)
            {
                if (SocketServer.Program.basedeviceid != _XD100xBuffer[i]._Info._id)
                {
                    //2012
                    if (_XD100xBuffer[i]._Info._version == "xd100n")
                    {
                        _XD100xBuffer[i]._Command[27]._cmd = Tool.xd100n.Set_outtemp(_XD100xBuffer[i]._Info._addr, _Set._outtemp);
                    }
                    else
                    {
                        //continue;
                        _XD100xBuffer[i]._Command[27]._cmd = Tool.xd100.Set_outtemp(_XD100xBuffer[i]._Info._addr, _Set._outtemp);
                    }
                    _XD100xBuffer[i]._Command[27]._senddt = DateTime.Now;
                    _XD100xBuffer[i]._Command[27]._onoff = true;
                }
            }
        }
        #endregion
