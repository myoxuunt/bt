...
namespace Tool
{
    class PrintDGV
    {
        ...
        #region method
        public static void Print_DataGridView(DataGridView dgv1, string mtitle)
        {
            ...
            try
            {
                ...
                PrintTitle = mtitle;//dlg.PrintTitle;
            ...
            try
            {
                ...
                while (RowPos <= dgv.Rows.Count - 1)
                {
                    ...
                    else
                    {
                        if (NewPage)
                        {
                            Font titlefont = new Font("Arial", 13);
                            e.Graphics.DrawString(PrintTitle, new Font(titlefont, FontStyle.Bold),
                                    Brushes.Black, e.MarginBounds.Left, e.MarginBounds.Top -
                            e.Graphics.MeasureString(PrintTitle, new Font(dgv.Font,
                                    FontStyle.Bold), e.MarginBounds.Width).Height - 15);

                            String s = "打印时间：" + DateTime.Now.ToLongDateString();// + " " + DateTime.Now.ToShortTimeString();

                            e.Graphics.DrawString(s, new Font(dgv.Font, FontStyle.Regular),
                                    ...
                                    FontStyle.Bold), FontStyle.Bold), e.MarginBounds.Width).Height - 8);
